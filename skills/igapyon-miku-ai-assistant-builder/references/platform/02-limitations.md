...
二段階フォルダ変換では、自動生成Knowledge sourceを最低1件確保するため、人力資料を最大19件とする。人力資料数を`M`、自動生成枠を`A = 20 - M`として、最終登録候補を20件以内に収める。これは本スキルの配分規則であり、Agent Builderが人力資料だけを19件に制限しているという意味ではない。
