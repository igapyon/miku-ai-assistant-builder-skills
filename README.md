...
新規変換の開始時には、配備先、入力元の正確なフォルダ、自動処理対象範囲を会話から解決します。利用者が直接指定した値だけでなく、過去の指定を引用した値、または利用者が示した事実から競合なく一意に類推できる値も使えます。採用値と根拠を復唱し、確認のためだけに待機しません。未解決、曖昧、複数候補、矛盾、または利用者の事実に裏付けられない推測だけは質問します。現在の作業ディレクトリ、リポジトリルート、開いているファイル、添付ファイル、ファイル名だけから入力元を決めることはありません。利用者が「特に指定なし」「おまかせ」と明示した項目は、目的に合う狭い範囲の想定で補完します。
